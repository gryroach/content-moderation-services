...
class JWTService:
    def __init__(self) -> None:
        private_key_path = SCRIPT_DIR / "keys" / "example_private_key.pem"
        public_key_path = SCRIPT_DIR / "keys" / "example_public_key.pem"

        with Path.open(private_key_path) as private_key_file:
            self.private_key = private_key_file.read()
        with Path.open(public_key_path) as public_key_file:
...
if __name__ == "__main__":
    user_id = input("Введите user_id в формате UUID, либо нажмите Enter, чтобы он был сгенерирован: ")
